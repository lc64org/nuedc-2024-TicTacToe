...
class TicTacToe:
	...
	def force_move(self, row: Literal[0, 1, 2], col: Literal[0, 1, 2], player: Literal[-1, 1]):
		"""强制落子"""
	...
	def try_update_board(self, new_board: list[list[int]]):
		...
		to_move_actions = [] # 通知机械臂撤销棋盘落子位置所需做出的动作 (from_row, from_col, target_row, target_col)，row == -1 代表从棋子堆中取棋子或将棋子放回棋子堆，在这种情况下 col 指示棋子类型
		...
		for removed in removed_pieces[:]:
			...
			for new_move in new_human_moves + new_computer_moves:
				...
				if removed[2] == new_board[new_move[0]][new_move[1]]:
					...
					to_move_actions.append((new_move[0], new_move[1], removed[0], removed[1]))
